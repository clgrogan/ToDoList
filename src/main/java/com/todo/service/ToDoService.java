...
public class ToDoService {
	...
	static {
		todos.add(new ToDo(nextToDoCount++, "curtg", "Master Core Java", LocalDate.now().plusDays(120), false));
		todos.add(new ToDo(nextToDoCount++, "curtg", "Spring Microservices", LocalDate.now().plusDays(240), false));
		todos.add(new ToDo(nextToDoCount++, "curtg", "Culinary Arts", LocalDate.now().plusDays(360), false));
	}
	...
	public ToDo getById(int id) {
		ToDo todo = todos.stream().filter(e -> e.getId() == id).findAny().orElse(null);
		return todo;
	}
